...
# Cria o diretorio base dos venvs Maya, por padrao seguir a tag da versao do AWX 
RUN for virtualenvs in $(ls -1 /tmp/Dockerfile_requirements/) ; \
    do \
        mkdir -p /opt/maya_venv/${virtualenvs} ; \
        chmod -R 0755 /opt/maya_venv ; \
    done
...
RUN for virtualenvs in $(ls -1 /tmp/Dockerfile_requirements/) ; \
    do \
        virtualenv --no-pip --system-site-packages /opt/maya_venv/${virtualenvs} ; \
    done

# Ativa o venv e instala todas as bibliotecas que o maya necessita
# Instala uma versao especifica para o pip
RUN for virtualenvs in $(ls -1 /tmp/Dockerfile_requirements/) ; \ 
    do \
    sh -c "source /opt/maya_venv/${virtualenvs}/bin/activate ; \
